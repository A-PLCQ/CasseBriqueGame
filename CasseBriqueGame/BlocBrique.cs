...
namespace CasseBriqueGame
{
    class BlocBrique
    {
        private int level = 0; // Niveau du bloc de briques
        public Brique[][] bloc; // Tableau de briques
        private int NB_COL = 10; // Nombre de colonnes de briques
        private int NB_LIGN = 4; // Nombre de lignes de briques
        ...
        {
            this.level = level;
            CreerNiveau(); // Initialise le niveau de briques
        }
        public void CreerNiveau ()
        {
            ...
            if (level == 1)
            {
                bloc[0][0] = new Brique(new Point(CasseBriqueGame.MARGE + 120, CasseBriqueGame.MARGE + 0), 3);
                bloc[0][1] = new Brique(new Point(CasseBriqueGame.MARGE + 150, CasseBriqueGame.MARGE + 0), 3);
